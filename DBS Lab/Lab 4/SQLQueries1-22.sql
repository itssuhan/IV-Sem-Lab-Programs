...
--Q3
select dept_name,count(course_id) as No_of_Courses from course group by dept_name;
...
--Q8
select max(sal_sum) from (select sum(salary) as sal_sum from instructor group by dept_name);
/*
...
--Q9
select name,avg_sal from(select name,avg(salary) as avg_sal,dept_name from instructor group by dept_name having avg(salary)>42000);
/*
...
--Q10
select max(no_of_enrol) from (select course_id,sec_id,count(ID) as no_of_enrol from section natural join takes where semester='Spring' and year=2010 group by course_id,sec_id);
/*
